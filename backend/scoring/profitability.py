...
def evaluate(model) -> Dict[str, Any]:
    rules = get_module_rules("profitability")
    ...
    for m in metrics:
        ...
        if hasattr(series, "iloc"):
            if agg == "last":
                try:
                    val = series.astype(float).iloc[0] if len(series) else None
                except Exception:
                    try:
                        val = series.astype(float).iloc[-1]
                    except Exception:
                        val = None
    ...
    rating = None
    return {"score": numeric, "rating": rating, "summary": "Profitability computed from configured metrics.", "evidence": evidence}
